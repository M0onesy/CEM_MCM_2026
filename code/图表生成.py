#!/usr/bin/env python3
"""
Indiana Fever 2025: Complete Chart Generation Script
...
def chart_01_player_value_decomposition():
    ...
    Stacked bar chart showing the breakdown of player values
    """
    fig, ax = plt.subplots(figsize=(12, 8))

    # Data
    players = ['Caitlin Clark', 'Aliyah Boston', 'Kelsey Mitchell', 'NaLyssa Smith', 'Lexie Hull']
    performance_value = [1.8, 0.9, 0.5, 0.3, 0.15]  # Million USD
    commercial_value = [15.2, 1.8, 0.45, 0.35, 0.12]  # Million USD
    ...
    for i, (perf, comm) in enumerate(zip(performance_value, commercial_value)):
        ...
        ax.text(i, perf / 2, f'${perf}M', ha='center', va='center',
                color='white', fontweight='bold', fontsize=10)
        # Commercial value label
        ax.text(i, perf + comm / 2, f'${comm}M', ha='center', va='center',
                color='white', fontweight='bold', fontsize=10)
        # Total label above bar
        ax.text(i, total + 0.5, f'Total: ${total}M', ha='center', va='bottom',
                fontweight='bold', fontsize=11)
        # Ratio label below
        ratio = comm / perf if perf > 0 else 0
        ax.text(i, -1.2, f'Ratio: 1:{ratio:.1f}', ha='center', va='top',
    ...
    plt.close()
    print("✓ Chart 01: Player Value Decomposition - Complete")
...
def chart_02_revenue_cost_structure():
    ...
    revenue_values = [27.9, 10.0, 6.0, 4.0, 2.5, 2.0]  # Total: $52.4M
    revenue_colors = [COLORS['teal'], COLORS['purple'], '#E67E22', '#E74C3C', COLORS['gray'], '#BDC3C7']

    # Cost data
    cost_labels = ['Operations', 'Arena', 'Marketing', 'Travel', 'Other', 'Payroll']
    cost_values = [8.0, 5.0, 3.0, 1.5, 1.3, 2.0]  # Total: $20.8M
    cost_colors = [COLORS['teal'], '#16A085', COLORS['gold'], '#F39C12', '#E74C3C', COLORS['dark_gray']]
    ...
    plt.savefig(f'{output_dir}/02_revenue_cost_structure.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
    print("✓ Chart 02: Revenue & Cost Structure - Complete")
...
def chart_05_wacc_optimization():
    ...
    r_d = 6 + 0.08 * debt_ratio + 0.002 * debt_ratio ** 2

    # Cost of equity increases faster with leverage
    r_e = 10 + 0.05 * debt_ratio + 0.003 * debt_ratio ** 2
    ...
    # Mark optimal point (minimum WACC)
    optimal_idx = np.argmin(wacc)
    optimal_debt = debt_ratio[optimal_idx]
    optimal_wacc = wacc[optimal_idx]

    ax1.plot(optimal_debt, optimal_wacc, 'k*', markersize=15, zorder=5)
    ax1.annotate(f'Optimal: {optimal_debt:.1f}%\nWACC: {optimal_wacc:.2f}%',
                 xy=(optimal_debt, optimal_wacc), xytext=(optimal_debt + 8, optimal_wacc - 0.8),
    ...
    plt.savefig(f'{output_dir}/05_wacc_optimization.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
    print("✓ Chart 05: WACC Optimization - Complete")
...
def chart_06_pricing_strategy():
    ...
    # Right: Dynamic pricing multiplier effect
    games = ['vs Chicago\n(Sat)', 'vs Vegas\n(Sun)', 'vs Atlanta\n(Tue)', 'vs Seattle\n(Wed)']
    base_price = 85

    # Multipliers
    opponent_mult = [1.4, 1.35, 1.0, 1.2]  # Star opponent effect
    day_mult = [1.2, 1.15, 0.9, 0.95]  # Day of week effect
    final_prices = [186, 178, 99, 126]  # After all multipliers
...
def chart_07_monte_carlo_distribution():
    ...
    # Parameters for profit distribution
    ...
    ax1.axvline(p5, color='orange', linestyle=':', linewidth=2, label=f'5th Percentile: ${p5:.1f}M')
    ax1.axvline(p95, color='green', linestyle=':', linewidth=2, label=f'95th Percentile: ${p95:.1f}M')
    ...
    ax2.annotate(f'VaR (95%): ${var_95:.1f}M', xy=(var_95, 5), xytext=(var_95 - 8, 18),
...
def chart_08_geographic_competition():
    ...
    distances = [112, 264, 290, 243, 287, 484, 592, 1367, 1227, 2336]  # Miles from Indianapolis
    revenue_impact = [-8.5, -5.2, -4.8, -4.0, -3.5, -2.8, -1.2, -0.6, -0.5, -0.2]  # Percentage impact
...
def chart_09_injury_impact():
    ...
    # Left: Waterfall chart for financial impact
    categories = ['Normal\nRevenue', 'Ticket\nLoss', 'Merch\nLoss', 'Sponsor\nImpact',
                  'TV Rating\nDrop', 'Net\nRevenue']
    ...
    # Injury probability model (exponential growth beyond threshold)
    base_risk = 5  # 5% base injury risk
    ...
    # Mark Clark's current usage and target
    clark_minutes = 35.8
    ...
    plt.savefig(f'{output_dir}/09_injury_impact.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
    print("✓ Chart 09: Injury Impact - Complete")
...
def chart_10_decision_state_machine():
    ...
    arrows = [
        ((3.2, 8), (4.8, 8)),  # Pre-season -> Early
        ((7.2, 8), (8.8, 8)),  # Early -> Mid
        ((10, 7.3), (10, 5.7)),  # Mid -> Trade Deadline
        ((7.2, 5), (8.8, 5)),  # Late -> Trade Deadline
        ((6, 6.3), (6, 5.7)),  # Early -> Late (down)
        ((4.8, 5), (3.2, 5)),  # Late -> Playoffs
        ((2, 6.3), (2, 5.7)),  # Pre -> Playoffs? No, Playoffs -> Off
        ((2, 4.3), (4.8, 2.3)),  # Playoffs -> Off-season
        ((6, 5 - 0.7), (6, 2 + 0.7)),  # Late -> Off-season
        ((6, 2 + 0.7), (6, 5 - 0.7)),  # Off-season -> Late (cycle back indicator)
    ]
...
def chart_12_sensitivity_tornado():
    ...
    # Variables and their impact (in Million USD)
...
def main():
    """Generate all charts"""
    print("\n" + "=" * 60)
    print("Indiana Fever 2025: Generating All Charts")
    print("=" * 60 + "\n")
